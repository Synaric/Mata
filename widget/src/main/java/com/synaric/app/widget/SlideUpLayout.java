...
public class SlideUpLayout extends ViewGroup {
    ...
    private int mHandleWidthOffset;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mVertical = true;
        mBottomOffset = 0;
        mTopOffset = (int) a.getDimension(R.styleable.SlideUpLayout_topOffset, 0.0f);
        mAllowSingleTap = true;
        mAnimateOnClick = true;
        mHandleWidthOffset = (int) a.getDimension(R.styleable.SlideUpLayout_handleWidthOffset, 0.0f);

        int handleId = a.getResourceId(R.styleable.SlideUpLayout_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.SlideUpLayout_content, 0);
        if (contentId == 0) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //mHandleWidthOffset用于handle不为match_parent的时候
        measureChild(
                handle,
                MeasureSpec.makeMeasureSpec(widthSpecSize - mHandleWidthOffset, MeasureSpec.EXACTLY),
                heightMeasureSpec
        );
...
//                    mTopOffset + childHeight + content.getMeasuredHeight());
    ...
    public CharSequence getAccessibilityClassName() {
        return SlideUpLayout.class.getName();
    }

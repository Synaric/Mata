...
public class SceneScaleUp extends TransitionAnims{
	...
	public void playScreenAnims(final boolean isEnter) {
		...
		if (isEnter) {
			...
			fromY = mStartY;
			toY = 0f;
			fromScaleX = mWidth / getSceneRoot().getWidth();
			toScaleX = 1f;
			fromScaleY = mHeight / getSceneRoot().getHeight();
			toScaleY = 1f;
		}else {
			fromAlpha = 1f;
			toAlpha = 0f;
			...
			toY = mStartY;
			fromScaleX = 1f;
			toScaleX = mWidth / getSceneRoot().getWidth();
			fromScaleY = 1f;
			toScaleY = mHeight / getSceneRoot().getHeight();
		}
